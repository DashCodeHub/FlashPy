...
class DataLoaderLite:
    def __init__(self, B, T, process_rank, num_processes):
        ...
        # state
        # self.current_position = 0 # now the current prosition wouldnot be zero
    ...
    def next_batch(self):
        ...
        # now we advance by B*T*self.num_processes
...
if ddp:
    ...
    # after you are doing the backward pass, it will call allreduce
...
# This doesnot work here in my system (windows), It needs linux
...
# model = GPT(GPTConfig()) # random model produces randome output
# model.eval()
# model.to(device)
...
# import tiktoken
# enc = tiktoken.get_encoding('gpt2')
# # Tiktokenizer working
# # 15496, 11, 314, 1101, 257, 3303, 2746, 11 inplace of "Hello, I'm a language model,"
...
while x.size(1) < max_length:
    # forward the model to the get the logits
    logits = model(x) # (B, T, vocab_size)
    # take the logits at the last position
    logits = logits[:, -1, :]
    # get the probability
    probs = F.softmax(logits, dim=-1)
    # do top-k smapling of 50 (hugging face dedault)
    # topk_probs herer becomes (5, 50) top_k indices is (5, 50)
    topk_probs, topk_indices = torch.topk(probs, 50, dim=-1)
    # select a token from top-k probabilities 
    ix = torch.multinomial(topk_probs, 1) # (B, 1)
    # gather the corresponsing indices
